...
//wsk[i].resultHex= wsk[i].dec2hex(wsk[i].resultString, 0);
	...
	plik.open("zakodowany.txt", ios::out|ios::app);
...
void cEncode::run(){
	...
	for(int k=0; i<ile_razy_wchodzi_16;i++){
		...
		wsk[i+1].ile_razy_wchodzi_16++; //chcyba niepotrzebne, nie wiem skad to wzialem
		}
	}
	if(nadmiaroweBity!=0){
...
void cEncode::iterations_decrypt(cKey o_key){
